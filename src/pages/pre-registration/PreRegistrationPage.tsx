...
export default function PreRegistrationPage() {
  ...
  const excelMutation = useMutation({
    mutationFn: async (file: File) => {
      const rows = await excelHelpers.parseExcelFile(file);
      if (!rows.length)
        throw new Error("No se encontraron filas con correos válidos");
      return preRegistrationService.bulkImport(orgId!, rows, eventId);
    },
    onSuccess: (result) => {
      queryClient.invalidateQueries({
        queryKey: ["pre-registered", orgId, eventId],
    ...
    onError: (error) => {
      setExcelError((error as any)?.message || "Error al procesar el archivo");
    },
  });
  ...
  const activated = (attendees as PreRegisteredAttendee[]).filter(
    (a) => a.isActivated,
  ).length;
  ...
  return (
    <>
      ...
      {bulkResult && (
        ...
        >
          <span style={{ fontSize: "0.875rem", color: "#15803d" }}>
            ✓ {bulkResult.inserted} correo{bulkResult.inserted !== 1 ? "s" : ""}{" "}
            importado{bulkResult.inserted !== 1 ? "s" : ""}.
            {bulkResult.skipped > 0 &&
              ` ${bulkResult.skipped} duplicado${bulkResult.skipped !== 1 ? "s" : ""} omitido${bulkResult.skipped !== 1 ? "s" : ""}.`}
          </span>
          <button
            className="btn"
      ...
      <div className="card" style={{ padding: 16, marginBottom: 16 }}>
        <h3 style={{ marginBottom: 12, fontSize: "0.9375rem" }}>
          Agregar correo
        </h3>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <input
            ...
            onKeyDown={(e) =>
              e.key === "Enter" && newEmail && addMutation.mutate()
            }
          />
          <input
            type="text"
          ...
          <button className="btn" onClick={() => setShowBulk(!showBulk)}>
            <Upload size={14} /> Importar masivo
          </button>
          <button
            className="btn"
            onClick={() => excelHelpers.downloadTemplate(event?.name)}
        ...
        <div style={{ marginTop: 8, display: "flex", gap: 8 }}>
          ...
          >
            <Upload size={14} />{" "}
            {excelMutation.isPending ? "Procesando..." : "Importar desde Excel"}
          </button>
        </div>
        ...
        {!showBulk && (
          <div style={{ marginTop: 12 }}>
            <button
              ...
              style={{ fontSize: "0.8125rem" }}
            >
              {showAdvanced ? "▼" : "▶"} Campos adicionales (opcional)
            </button>
            {showAdvanced && (
              <div
                style={{
              ...
              >
                <input
                  type="text"
                  placeholder="Canal/Organización"
                  value={newChannel}
        ...
        {showBulk && (
          <div style={{ marginTop: 16 }}>
            ...
            >
              Un correo por línea. Formato opcional:{" "}
              <code>correo@ejemplo.com, Nombre</code>
            </p>
            <textarea
              value={bulkText}
      ...
      {isLoading ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              style={{ height: 48, background: "#f1f5f9", borderRadius: 10 }}
            />
          ))}
        </div>
      ) : attendees.length === 0 ? (
        ...
        >
          ...
          />
          <p style={{ margin: 0 }}>
            No hay correos pre-registrados. Agrega el primero arriba.
          </p>
        </div>
      ) : (
              ...
              <tbody>
                {(attendees as PreRegisteredAttendee[]).map((a, idx) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "10px 14px" }}>{fmt(a.channel)}</td>
                    <td style={{ padding: "10px 14px" }}>{fmt(a.position)}</td>
                    <td style={{ padding: "10px 14px" }}>{fmt(a.country)}</td>
                    <td
                      style={{
                        padding: "10px 14px",
                        maxWidth: "200px",
                    ...
                    <td style={{ padding: "10px 14px", whiteSpace: "nowrap" }}>
                      {deleteConfirmId === a._id ? (
                        ...
                        <button
                          ...
                          title={
                            a.isActivated
                              ? "No se puede eliminar un correo ya activado"
                              : "Eliminar"
                          }
                        >
                          <Trash2 size={12} />
